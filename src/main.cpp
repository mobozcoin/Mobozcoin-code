...
#if defined(NDEBUG)
# error "MobozCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x44e2276a790e527d9062267573002b0567544d9c15822fa4c1e2c0033f4e5c44");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MobozCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MobozCoin Signed Message:\n";
...
{
    // MobozCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // MobozCoin: 840k blocks in ~4 years

    // return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 5; // MobozCoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // MobozCoin: 30 secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MobozCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MobozCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x329a5eca7bf1acbf93fa7e8deb4fc2c444aa24ecb09d3a5ddc334442699488c5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mbc genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1507635422;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 273349;

        if (fTestNet)
        {
            block.nTime    = 1492412282;
            block.nNonce   = 973697;
        }
 ...
 // If genesis block hash does not match, then generate new genesis hash.
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x782e75c29f82696649744c3eb148fc1b85337e812511882801a0fe115e356200"));
...
unsigned char pchMessageStart[4] = {  0xda, 0xcf, 0xae, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MobozCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MobozCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MobozCoinMiner(CWallet *pwallet)
{
    printf("MobozCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MobozCoin-miner");
    ...
    try { loop {
        ...
        printf("Running MobozCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MobozCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MobozCoinMiner, pwallet));
}
